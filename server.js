...
app.use(express.static(__dirname)); // Lê os arquivos da pasta atual

app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'index.html')); // Força a abertura do jogo
});
// ----------------------------
...
io.on('connection', (socket) => {
    socket.on('joinRoom', (roomID) => {
        ...
        if (rooms[roomID].players.length < 2) {
            ...
            if (rooms[roomID].players.length === 2) {
                io.to(roomID).emit('startGame', 'red'); 
            }
        } else {
